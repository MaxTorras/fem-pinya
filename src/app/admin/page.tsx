...
// Styled components for tabs & selectors
  ...
  return (
    <div className="flex gap-2 mb-4">
      {tabs.map(tab => (
        ...
        >
          {tab === "attendance" ? "Attendance" : tab === "members" ? "Members" : "By Position"}
        </button>
      ))}
    </div>
  );
...
function DaySelector({ dates, selectedDate, setSelectedDate }: { dates: string[]; selectedDate: string; setSelectedDate: (date: string) => void }) {
...
export default function AdminPage() {
  ...
  const attendanceDates = Array.from(new Set(attendance.map(r => r.date))).sort((a, b) => new Date(b).getTime() - new Date(a).getTime());
  ...
  const membersByPosition = useMemo(() => {
    return attendance.filter(r => r.date === selectedDate).reduce<Record<string, Member[]>>((acc, record) => {
      const member = members.find(m => m.nickname.toLowerCase() === record.nickname.toLowerCase());
  ...
  if (!loggedIn) {
    return (
      <div className={`${quicksand.className} flex flex-col items-center justify-center min-h-screen gap-4 bg-white`}>
        <h1 className="text-3xl font-bold text-[#2f2484]">Admin Login</h1>
        ...
        <button onClick={handleLogin} className="bg-[#2f2484] text-yellow-400 px-6 py-2 rounded font-semibold hover:bg-yellow-400 hover:text-[#2f2484] transition">
          Enter
        </button>
  ...
  return (
    <main className={`${quicksand.className} p-6 max-w-3xl mx-auto bg-white min-h-screen`}>
      <h1 className="text-3xl font-bold text-[#2f2484] mb-6">Admin Dashboard</h1>

      <Tabs tabs={["attendance","members","positions"]} activeTab={activeTab} setActiveTab={setActiveTab} />
      ...
      {loading ? (
        ...
        <>
          <ul className="border-2 border-[#2f2484] rounded divide-y mb-6">
            {Object.entries(groupedAttendance).sort((a,b) => new Date(b[0]).getTime()-new Date(a[0]).getTime())
              .map(([date,count]) => (
        ...
        <ul className="border-2 border-[#2f2484] rounded divide-y">
          {members.map(m => (
            <li key={m.nickname} className="p-3">{m.name} {m.surname} <span className="text-gray-600 text-sm">({m.nickname})</span></li>
          ))}
        </ul>
      ) : (
          ...
          {Object.keys(membersByPosition).length === 0 ? (
            ...
            <div key={pos} className="border-2 border-[#2f2484] rounded p-3">
              ...
              <ul className="text-sm text-gray-700 list-disc list-inside">
                {list.map(m => <li key={m.nickname}>{m.name} {m.surname} ({m.nickname})</li>)}
              </ul>
            </div>
          ))}
        </div>
